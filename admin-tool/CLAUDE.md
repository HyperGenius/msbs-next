...
- 武器スロット数（`weapon_slot_count`）は `mobile_suits.weapon_slot_count` → 機体名で引いた機体マスター → `max(装備数, 2)` の順に解決する
  （`resolve_weapon_slot_count()`、Issue #543）。プレイヤー機は NULL のまま機体マスターから解決する。
  NPC 機は機体名が `"{機体マスター名} (NPC)"` で機体マスターを引けないため、機体ごとに値を持つ。エース機は雛形の `mobile_suit.weapon_slot_count` をマッチング時にコピーする。
- NPC 機・エース機の武装は、本数がスロット数以下で、武器 ID が重複しないこと（弾数・クールダウンを武器 ID で管理するため）。
  ビームジェネレータLv の条件は適用しない。
