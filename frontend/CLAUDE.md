...
| モーダルが BottomNav の裏に隠れる | `BottomNav` は `z-50 h-16 fixed bottom-0 md:hidden`。モーダルは `z-[60]` 以上にすること。また `items-center p-4` で上下均等マージンを確保すること。可能なら独自に `fixed inset-0` を書かず共通コンポーネント `components/ui/SciFiModal.tsx`（`z-[60]` 固定、`max-h-[85dvh]` + `overflow-y-auto`、Esc/オーバーレイクリックで閉じる実装済み）を使う。過去に `CustomizationModal.tsx`（`z-40`）と、その上に重ねて開く `WeaponChangeModal.tsx`（`z-50`）が `SciFiModal` 導入前の実装のまま取り残され、この規約を満たしておらずBottomNavの裏に隠れる不具合になったことがある（Issue #413で `z-[60]`/`z-[70]` に修正）。`BattleDetailModal.tsx`（`history/`配下、独自に `fixed inset-0` を書いている）でも同じ理由で `z-50` のまま取り残されていたため `z-[60]` に修正した（Issue #423）。モーダルの上にさらにモーダルを重ねる場合は、外側より高い `z-index`（例: `z-[70]`）にすること |
| Battle History一覧（`/history`）でページ全体とRecordsカード内部の2箇所が同時にスクロールする | `frontend/src/app/history/page.tsx` のページルート要素を `min-h-full` のまま `BattleList.tsx` 側に固定 `max-h-[800px] overflow-y-auto` を持たせていたため、モバイル幅（`main` が `h-[100dvh]` で高さ固定される範囲）では `main` 自体のスクロールとRecordsカード内部のスクロールが二重に発生していた。修正では、ページルート要素を `h-full overflow-hidden md:h-auto md:overflow-visible flex flex-col` にして `main`（モバイル時は高さ固定）にぴったり合わせ、`BattleList.tsx` 側は固定 `max-h` をやめて `flex-1 min-h-0 overflow-y-auto` に変更した。これによりモバイルではRecordsカードのみがスクロールし（ページ自体はスクロール不要になる）、デスクトップ幅（`md:overflow-visible` でmainの高さ固定が外れる）では逆にRecordsカードは伸びるだけでスクロールせず、ウィンドウ側のスクロールのみが機能する（＝どちらの幅でもスクロール領域は常に1箇所）。新しく「一覧+詳細」系のページを作る際、内側に `max-h-[...]px` の固定値でスクロール領域を作ると、ページ側の高さが伸縮する限り同じ二重スクロールを再発する可能性があるため注意（Issue #423, PR #424） |
...
| 未ログイン状態で `/garage` 等の保護ルートにアクセスすると 404 になる | Clerkのルート保護（`protect-rewrite`）による意図した挙動。このリポジトリにはClerkテストトークンを使ったE2E（Playwright等）の仕組みが無い。どうしても実データに近い見た目をスクリーンショットで確認したい場合は、①モックpropsを渡すだけの一時ページ（例: `src/app/digest-preview/page.tsx`）を作る、②`middleware.ts` の `isPublicRoute` に一時的にそのパスを追加する、③ローカルdevサーバー + Playwright（`node -e` でも可、`chromium-cli` があればそちら）でスクリーンショットを撮る、④確認後に一時ページと `middleware.ts` の変更を必ず削除・`git checkout --` で元に戻す、という手順で対応できる（Issue #415 で実施）。恒常的な確認手段が必要になった場合のみ、Clerkテスト用の仕組みを別途検討する |
| sticky要素の直下・周囲がスクロール時に透けて重なって見える | 疑うべき箇所は2つ。①sticky要素自身の `mb-*`（margin）は背景色でカバーされないため、その隙間から下のコンテンツが透けて見える → `mb-*` ではなく `pb-*`（padding）にする。②**それでも直らない場合は親コンテナ側のpaddingを疑う**: 親に `p-*` を付けると、sticky要素（`top-0`）はそのpadding分だけ「コンテナ上端より下にずれた位置」で止まるため、コンテナ上端〜sticky要素の間に隙間ができ、そこをスクロール中のコンテンツが突き抜けて見える。この場合はpaddingを親から外し、sticky要素自身に持たせてコンテナの最上端に完全密着させ、他の子要素側に個別にpaddingを付け替える（`BattleList.tsx` の「Records」見出しで実際に①→②の順で2段階の修正が必要だった。Issue #415, PR #416 → #417）。念のため `z-10` 等の明示的な `z-index` も付けておくと安全。修正後は `elementFromPoint()` で重なり疑わしい座標のヒットテストを取るか、実データに近い件数・複数スクロール位置でPlaywrightスクリーンショットを撮って確認するのが確実 |
| BattleViewerの照準線（`TargetLine`）・射線（`TracerMesh`。Issue #531 以前は `AttackLine`）が実際のユニット位置と無関係な方向を指す | `useBattleSnapshot.ts` の `getBattleSnapshot()` が `pos = 直近position_snapshot + velocity_snapshot × dt` でログ間の位置を外挿している（`BattleScene.tsx` の各ラインはこの `pos` をそのまま使う）。`velocity_snapshot` はほぼ毎ティック付与されるが、撃破時の `DESTROYED` ログ（`backend/app/engine/combat.py`）には含まれないため、撃破後や長時間移動が無い区間では古い速度ベクトルのまま `dt` が伸び続け、実位置から大きくズレて外挿されてしまう。`getBattleSnapshot()` 内の `MAX_VELOCITY_EXTRAPOLATION_SECONDS`（Issue #421）で `dt` に上限を設け、超過時は外挿せず直近の実位置に留める形で修正済み。ライン描画系のロジックを追加・変更する際はこの `dt` 上限の前提を崩さないこと。詳細は `docs/features/battle-viewer-improvement.md` を参照 |
