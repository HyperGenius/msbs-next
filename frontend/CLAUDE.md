...
| sticky要素の直下がスクロール時に透けて重なって見える | `sticky`要素に `mb-*`（margin）で下との余白を作ると、marginは要素の背景色でカバーされないため、その隙間から下のコンテンツがスクロールで透けて見える（例: `BattleList.tsx` の「Records」見出し、Issue #415）。sticky要素の余白は `mb-*` ではなく `pb-*`（padding）にして背景色の内側に収めること。念のため `z-10` 等の明示的な `z-index` も付けておくと安全 |
