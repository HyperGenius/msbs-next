...
describe("allocateStatusPoints", () => {
  it("成功時に更新後のパイロット情報を返す", async () => {
    vi.mocked(fetch).mockResolvedValue(mockOk({ ...mockPilot, dex: 8 }));

    const result = await allocateStatusPoints({ dex: 3 });
  ...
  it("未指定のステータスは0として送信する", async () => {
    ...
    await allocateStatusPoints({ dex: 2 });
  ...
  it("ポイント不足の場合にdetailメッセージで例外を投げる", async () => {
    ...
    await expect(allocateStatusPoints({ dex: 99 })).rejects.toThrow(
      "割り振り可能なポイントが不足しています"
    );
  });
});
