...
#### ステータスポイント割り振り可能ステータス（6種）

| キー | 名称 | 効果 |
|------|------|------|
| SHT | 射撃精度 | 射撃攻撃力補正率（シグモイド入力）|
| MEL | 格闘技巧 | 格闘攻撃力補正率（シグモイド入力）|
| INT | 直感 | クリティカル率・回避率 |
| REF | 反応 | イニシアチブ・機動性ボーナス |
| TOU | 耐久 | ダメージ加算・被クリティカル低下・防御加算 |
| LUK | 幸運 | ダメージ乱数偏り・完全回避 |

> **Note**: Phase E-1 (2026-05-17) にて `DEX` ステータスを廃止し `SHT` と `MEL` に置換。既存ユーザーの `dex` 値は両方にコピー済み。
