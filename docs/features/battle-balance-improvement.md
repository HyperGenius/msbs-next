...
**作成日:** 2026-05-17
**更新日:** 2026-05-24
**ステータス:** Phase E-1・E-2・E-3・E-3.5 実装完了 / Phase E-4〜E-6 設計中
...
> **Phase E-3 実装済み:** 攻撃角度によるセクタ補正（命中率・ダメージ）および BattleLog への `attack_sector` 記録は完了。
...
攻撃角度セクタ補正（3.4）と連動する戦術パッシブスキル。このスキルを持つパイロットは、ターゲットの背面・側面後方（`REAR_SIDE` / `REAR` セクタ）への積極的な位置取りを試みる。スキルレベルに応じて発動確率が変わり、発動にはブーストエネルギーを消費する。

#### スキルレベルと発動確率

| スキルレベル | 発動確率 | 想定パイロット像 |
|---|---|---|
| なし | 5% | 初心者（偶発的な後方取得） |
| Lv.1 | 30% | 経験者 |
| Lv.2 | 60% | ベテラン |
...
スキルなしでも低確率で発動することで、初心者がベテランに対して偶発的に背後を取れる余地を残し、「絶対に勝てない」状況を回避する。

#### 発動時の挙動

発動時、ユニットはターゲットの `REAR` 方向へのベクトルをポテンシャルフィールドの引力として追加する。

```python
# potential_field.py への追加（概案）
def _flanking_attraction(self, unit, target, target_heading_deg: float) -> np.ndarray:
    """フランキングスキル発動時のターゲット背面方向への引力ベクトルを返す."""
    target_heading_rad = math.radians(target_heading_deg)
    # ターゲット正面方向の逆ベクトル（背面方向）
    rear_dir = np.array([-math.cos(target_heading_rad), 0.0, -math.sin(target_heading_rad)])
    rear_target_pos = target.position.to_numpy() + rear_dir * FLANKING_OFFSET_DISTANCE
    delta = rear_target_pos - unit.position.to_numpy()
    dist = np.linalg.norm(delta) + 1e-6
    return (delta / dist) * FLANKING_ATTRACTION_WEIGHT
```

#### エネルギー消費（ブーストシステム共有）

フランキング移動はブーストシステムと同一リソース（`boost_energy`）を消費する。エネルギーが不足している場合は発動しない。戦場でのガス欠リスクが戦術的なトレードオフになる。

| 条件 | 挙動 |
|---|---|
| `boost_energy >= FLANKING_ENERGY_COST` | フランキング発動・エネルギー消費 |
| `boost_energy < FLANKING_ENERGY_COST` | 発動キャンセル・通常ポテンシャルフィールドで移動 |
| `boost_energy == 0`（ガス欠） | 移動速度低下・フランキング不可 |

`FLANKING_ENERGY_COST` はブースト1回分の消費量（`BOOST_ENERGY_COST`）の定数倍で定義し、バランス検証ツールで調整する。

#### 3D空間における移動制約（バトルフィールド依存）

戦場の `surface_type` によって上下軸方向の制約が異なる。この制約はフランキングスキル固有ではなく、**ポテンシャルフィールド全体の課題**（`BattleField.surface` と軸方向斥力関数の設計）として別途対応する。フランキングの引力ベクトルはこの制約を上書きしない（ポテンシャルフィールドの合成結果に従う）。

| `surface_type` | 上下軸制約 | 備考 |
|---|---|---|
| `SPACE` | 制限なし（3D自由移動） | 上下フランキングが最も有効 |
| `GROUND`（地球・月面） | 上方向に制限あり | 地面への斥力で下方向も制限 |
| `UNDERWATER` | 下方向に制限あり | 海底への斥力で下方向を制限 |

#### 新定数（`constants.py`）

```python
FLANKING_OFFSET_DISTANCE: float = 30.0    # 背面ターゲット位置のオフセット距離（要チューニング）
FLANKING_ATTRACTION_WEIGHT: float = 1.5   # フランキング引力の重み（要チューニング）
FLANKING_ENERGY_COST_RATE: float = 1.5    # BOOST_ENERGY_COST に対する消費倍率
FLANKING_SKILL_PROBABILITIES: dict[int, float] = {
    0: 0.05,   # スキルなし
    1: 0.30,   # Lv.1
    2: 0.60,   # Lv.2
    3: 0.90,   # Lv.3
...
#### 3.6.3 `PilotStats` の参照元 ✅ Phase E-2 実装済み
...
加えて攻撃角度 [3.4]・フランキングスキル [3.5]・パイロット能力 [3.6] が組み合わさることで、同じ MS でも「パイロットの腕と機体の位取り」によって結果が変わる戦術的な深みが生まれる。
...
- `FLANKING_OFFSET_DISTANCE` と `FLANKING_ATTRACTION_WEIGHT` は戦場スケール・ユニット速度に依存するため、バランス検証ツールで軌跡を可視化しながら調整する
- スキルレベル別発動確率（5% / 30% / 60% / 90%）はプレイテストで「偶発的な背後取得」が過多にならないか確認する
- `FLANKING_ENERGY_COST_RATE` が高すぎるとスキル持ちパイロットでもガス欠が頻発し形骸化するため、ブースト消費全体のバランスと合わせて検討する
- 将来: 「背後を取られない立ち回り」（後方への斥力、振り向き優先行動）はポテンシャルフィールドへの追加として別 Issue で対応

### 5.4.1 3D空間の軸制約（BattleField・ポテンシャルフィールド課題）

フランキングの引力ベクトルは `surface_type` による上下軸制約を受ける（[3.5] 参照）。この制約はフランキング固有ではなく、`BattleField.surface` と斥力関数の設計問題として以下を検討する。

- `SPACE`: 制約なし。上下方向のフランキングが発生するため、3D軸での引力ベクトル正規化が必要
- `GROUND`（地球・月面）: 重力相当の下方向斥力 + 地形上限斥力で上下を制限
- `UNDERWATER`: 海底斥力で下方向を制限。浮力相当の上方向バイアスも検討
...
| **Phase E-2** | パイロット能力の全ユニット（NPC含む）適用 [3.6] ✅ | 高 | E-1 |
| **Phase E-3** | 攻撃角度によるセクタ補正 [3.4] ✅ | 中 | heading_deg（既実装） |
| **Phase E-3.5** | フランキングスキル（パッシブ）[3.5] ✅ | 中 | E-3・ブーストシステム |
| **Phase E-4** | ビジー状態システム [3.2] | 中 | E-3.5（ターゲット選択変更） |
