...
3. ターゲットが `MELEE_BOOST_ARRIVAL_RANGE`（`MELEE_RANGE × 2`）内に入った
...
> **補足:** 「使用予定の遠距離武器」は低階層ファジィ推論の武器選択で決定した武器の `max_range` を参照する。EN / 弾薬が枯渇していて遠距離武器が使用不可の場合はブースト継続（格闘圏まで詰める）。
