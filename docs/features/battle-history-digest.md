...
生還 / 撃破: 3機 / 搭乗機: Gelgoog
獲得Credits: 1,200 / 獲得Exp: 350
...
「被弾: 中破」等の被弾ラベルは一言ダイジェスト（`digest_text`）の内容と情報が重複するため削除した（辛勝タグ時の「HP最低」表示は維持）。代わりに `BattleResult.credits_gained` / `exp_gained` を要約行に追加し、獲得報酬が一覧から一目でわかるようにした（Issue #423）。
