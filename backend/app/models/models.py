...
class Weapon(SQLModel):
    ...
    max_ammo: int | None = Field(default=None, description="最大弾数 (Noneまたは0の場合は無限/EN兵器)")
...
class MobileSuit(SQLModel, table=True):
    ...
    max_en: int = Field(default=1000, description="最大エネルギー容量 (ジェネレーター出力)")
    en_recovery: int = Field(default=100, description="ターン毎のEN回復量")
    max_propellant: int = Field(default=1000, description="最大推進剤容量 (将来的な移動コスト用)")
