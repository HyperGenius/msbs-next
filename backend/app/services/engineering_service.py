...
class EngineeringService:
    ...
    def calculate_upgrade_cost(
        stat_type: str, current_value: int | float
    ) -> int:
    ...
    def _validate_and_get_cost(
        ...
        if stat_type == "hp":
            ...
            if current_value >= self.MAX_HP_CAP:
                raise ValueError(
                    f"HP is already at maximum ({self.MAX_HP_CAP})"
                )
        elif stat_type == "armor":
            current_value = ms.armor
            if current_value >= self.MAX_ARMOR_CAP:
                raise ValueError(
                    f"Armor is already at maximum ({self.MAX_ARMOR_CAP})"
                )
        elif stat_type == "mobility":
            ...
            current_value = first_weapon["power"] if isinstance(first_weapon, dict) else first_weapon.power
        ...
        if stat_type == "hp":
            ...
            for weapon in weapons:
                if isinstance(weapon, dict):
                    ...
                    weapon["power"] = min(new_power, self.MAX_WEAPON_POWER_CAP)
    ...
    def upgrade_stat(
        ...
        ms_id = uuid.UUID(mobile_suit_id) if isinstance(mobile_suit_id, str) else mobile_suit_id
    ...
    def get_upgrade_preview(
        self, mobile_suit_id: str, stat_type: str
    ) -> dict:
        ...
        if stat_type == "hp":
            ...
            current = ms.mobility
            new_value = min(
                current + self.MOBILITY_INCREASE, self.MAX_MOBILITY_CAP
            )
            cost = self.calculate_upgrade_cost(stat_type, current)
            ...
            first_weapon = ms.weapons[0]
            current = first_weapon["power"] if isinstance(first_weapon, dict) else first_weapon.power
            new_value = min(
