...
def create_test_unit(name: str, pos: Vector3, side: str, terrain_adapt: dict) -> MobileSuit:
...
def test_detection_mechanics():
    ...
    print(f"  Close enemy distance: 300m")
    print(f"  Far enemy distance: 800m")
    
    # Run detection phase
    sim._detection_phase()
    
    print(f"\n  Detected enemies (PLAYER team): {len(sim.team_detected_units['PLAYER'])}")
    print(f"  Close enemy detected: {enemy_close.id in sim.team_detected_units['PLAYER']}")
...
def test_full_battle_simulation():
    ...
    print(f"  Player: Space Gundam (GROUND: D)")
    print(f"  Enemy: Ground Zaku (GROUND: S)")
