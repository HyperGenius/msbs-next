...
def test_batch_creates_next_open_room(session: Session):
    ...
    # SQLite returns naive datetimes; normalize both sides for comparison
    stored_time = new_open_room.scheduled_at
    if stored_time.tzinfo is None:
        stored_time = stored_time.replace(tzinfo=UTC)
    assert stored_time == expected_time, (
...
def test_api_always_returns_next_room(session: Session):
...
def test_no_duplicate_open_rooms(session: Session):
