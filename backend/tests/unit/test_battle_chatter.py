...
def create_test_unit(
    name: str, side: str, personality: str | None = None
) -> MobileSuit:
...
def test_ace_destruction_log():
    ...
    if len(destruction_logs) > 0:
        ...
        ace_destruction = next(
            (log for log in destruction_logs if "★【エース撃破】" in log.message), None
        )

        # If ace was destroyed, verify special message
        if ace.current_hp <= 0:
            assert ace_destruction is not None, (
                "Ace destruction should have special message"
            )
