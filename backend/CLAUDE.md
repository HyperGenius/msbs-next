...
### flaky（不安定）なモンテカルロ的アサーションのテストは削除せず `xfail` 化する

戦闘シミュレーション系のテスト（`tests/unit/test_phase_d_integration.py` 等）には、ファジィ推論による
AI の行動選択や乱数に依存し、複数回試行しても一定確率で失敗する「モンテカルロ的アサーション」が存在する
（例: `test_boost_start_occurs_with_full_field`、Issue #519）。

このようなテストが flaky であることが判明した場合、**安易に削除しない**こと。まず、検証対象の機構
（例: BOOST_START の発火ロジック自体）が、同ファイル内の決定論的なテスト（`current_action` を直接設定する等
してファジィ推論をバイパスするテスト。例: `test_boost_start_triggered_via_engage_melee`）で別途カバーされて
いるか確認する。

- カバーされている場合: 統合シナリオとしての意図（「複雑な条件下でも機構が機能する」ことのドキュメント）を
  残すため、`@pytest.mark.xfail(reason="...", strict=False)` を付与し、flaky になった理由と代替の決定論的
  テスト名を `reason` に明記する（`test_boost_start_occurs` の実装例を参照）。`strict=False` により、稀に
  成功しても CI を落とさない。
- カバーされていない場合: 決定論的な検証手段（アクションを直接設定してバイパスする等）を先に追加してから、
  上記と同様に `xfail` 化するか、シナリオパラメータ（試行回数・ターン数等）を見直す。
